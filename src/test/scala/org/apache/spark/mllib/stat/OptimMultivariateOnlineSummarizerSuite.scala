...
class OptimMultivariateOnlineSummarizerSuite extends SparkFunSuite {

  test("basic error handing") {
    val summarizer = new OptimMultivariateOnlineSummarizer
    ...
    val summarizer2 = (new OptimMultivariateOnlineSummarizer).add(Vectors.dense(1.0, -2.0, 0.0, 4.0))
  ...
  test("merging two summarizers") {
    val summarizer1 = (new OptimMultivariateOnlineSummarizer)
      .add(Vectors.sparse(3, Seq((0, -2.0), (1, 2.3))))
      .add(Vectors.dense(0.0, -1.0, -3.0))

    val summarizer2 = (new OptimMultivariateOnlineSummarizer)
      .add(Vectors.sparse(3, Seq((1, -5.1))))
      .add(Vectors.dense(3.8, 0.0, 1.9))
      .add(Vectors.dense(1.7, -0.6, 0.0))
      .add(Vectors.sparse(3, Seq((1, 1.9), (2, 0.0))))
  ...
  test("merging summarizer with empty summarizer") {
    ...
    val summarizer1 = (new OptimMultivariateOnlineSummarizer)
      .add(Vectors.dense(0.0, -1.0, -3.0)).merge(new OptimMultivariateOnlineSummarizer)
    assert(summarizer1.count === 1)

    val summarizer2 = (new OptimMultivariateOnlineSummarizer)
      .merge((new OptimMultivariateOnlineSummarizer).add(Vectors.dense(0.0, -1.0, -3.0)))
    assert(summarizer2.count === 1)

    val summarizer3 = (new OptimMultivariateOnlineSummarizer).merge(new OptimMultivariateOnlineSummarizer)
  ...
  test("merging summarizer when one side has zero mean (SPARK-4355)") {
    val s0 = new OptimMultivariateOnlineSummarizer()
      .add(Vectors.dense(2.0))
      .add(Vectors.dense(2.0))
    val s1 = new OptimMultivariateOnlineSummarizer()
  ...
  test("test min/max with weighted samples (SPARK-16561)") {
    ...
    val summarizer3 = new OptimMultivariateOnlineSummarizer()
    for (i <- 1 to 100) {
