 ...
 */
@Since("1.2.0")
class MyLogisticRegression @Since("1.2.0")(
                                            @Since("1.4.0") override val uid: String)
  extends ProbabilisticClassifier[Vector, MyLogisticRegression, MyLogisticRegressionModel]
 ...
 *
  ...
  private var optInitialModel: Option[MyLogisticRegressionModel] = None

  /** @group setParam */
  private[spark] def setInitialModel(model: MyLogisticRegressionModel): this.type = {
    this.optInitialModel = Some(model)
    this
  }

  override protected[spark] def train(dataset: Dataset[_]): MyLogisticRegressionModel = {
  ...
  protected[spark] def train(dataset: Dataset[_], handlePersistence: Boolean):
    ...
    val (coefficients, intercept, objectiveHistory) = {
      ...
      if (numClasses > 2) {
        ...
        val costFun = new MyLogisticCostFun(instances, numClasses, $(fitIntercept),
        ...
        while (states.hasNext) {
          ...
          logInfo(s"LOR iteration ${state.iter} finished, spends ${endTime - startTime} ms.")
        }
    ...
    val model = copyValues(new MyLogisticRegressionModel(uid, coefficients, intercept))
...
/**
 * Model produced by [[MyLogisticRegression]].
 */
@Since("1.4.0")
class MyLogisticRegressionModel private[spark](
    @Since("1.4.0") override val uid: String,
    @Since("2.0.0") val coefficients: Vector,
    @Since("1.3.0") val intercept: Double)
  extends ProbabilisticClassificationModel[Vector, MyLogisticRegressionModel]
  ...
  private[classification] def findSummaryModelAndProbabilityCol():
      (MyLogisticRegressionModel, String) = {
 ...
 *
  ...
  override def copy(extra: ParamMap): MyLogisticRegressionModel = {
    val newModel = copyValues(new MyLogisticRegressionModel(uid, coefficients, intercept), extra)
   ...
   *
   * For [[MyLogisticRegressionModel]], this does NOT currently save the training [[summary]].
   * An option to save [[summary]] may be added in the future.
   *
   * This also does not save the [[parent]] currently.
   */
  @Since("1.6.0")
  override def write: MLWriter = new MyLogisticRegressionModel.LogisticRegressionModelWriter(this)
}
...
object MyLogisticRegressionModel extends MLReadable[MyLogisticRegressionModel] {
  ...
  @Since("1.6.0")
  override def read: MLReader[MyLogisticRegressionModel] = new LogisticRegressionModelReader

  @Since("1.6.0")
  override def load(path: String): MyLogisticRegressionModel = super.load(path)

  /** [[MLWriter]] instance for [[MyLogisticRegressionModel]] */
  private[MyLogisticRegressionModel]
  class LogisticRegressionModelWriter(instance: MyLogisticRegressionModel)
  ...
  private class LogisticRegressionModelReader
    extends MLReader[MyLogisticRegressionModel] {

    /** Checked against metadata when loading model */
    private val className = classOf[MyLogisticRegressionModel].getName

    override def load(path: String): MyLogisticRegressionModel = {
      ...
      val Row(numClasses: Int, numFeatures: Int, intercept: Double, coefficients: Vector) =
        MLUtils.convertVectorColumnsToML(data, "coefficients")
          ...
          .head()
      val model = new MyLogisticRegressionModel(metadata.uid, coefficients, intercept)
 ...
 */
private class MyLogisticAggregator(
   ...
   * @return This LogisticAggregator object.
   */
  def merge(other: MyLogisticAggregator): this.type = {
 ...
 */
private class MyLogisticCostFun(
  ...
  override def calculate(coefficients: BDV[Double]): (Double, BDV[Double]) = {
    ...
    val logisticAggregator = {
      val seqOp = (c: MyLogisticAggregator, instance: Instance) =>
        c.add(instance, coeffs, localFeaturesStd)
      val combOp = (c1: MyLogisticAggregator, c2: MyLogisticAggregator) => c1.merge(c2)

      instances.treeAggregate(
        new MyLogisticAggregator(numFeatures, numClasses, fitIntercept)
      )(seqOp, combOp)
    }
